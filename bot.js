...
const { init: initPoller, trackOrder }         = require("./poller");
const {
...
// ─── Env validation ───────────────────────────────────────────────────────────
const REQUIRED_ENV = ["BOT_TOKEN", "API_KEY", "API_URL", "KORA_SECRET_KEY", "RENDER_URL", "DATABASE_URL"];
...
const ADMIN_ID        = 5549614582; // ChickletBoost admin Telegram ID

// ─── Bot initialisation ───────────────────────────────────────────────────────
const bot = new Telegraf(process.env.BOT_TOKEN);
bot.use(session());
setBotInstance(bot); // share bot with webhook.js for user notifications
initPoller(bot);     // start background order poller
...
const bal = await getBalance(uid(ctx));
  ...
  await query(
    ...
    [uid(ctx), ctx.from.username || null, ctx.from.first_name || null]
  );
...
bot.action("ACT:SERVICES", async (ctx) => {
  ...
  try {
    ...
    const lines    = preview.map(
      (s) =>
        `📌 <b>${s.name}</b>\n` +
        `   ${fmtRate(s.rate)} | Min: ${Number(s.min).toLocaleString("en-NG")} | Max: ${Number(s.max).toLocaleString("en-NG")}`
    );
...
bot.on("text", async (ctx) => {
  ...
  switch (s.step) {
    ...
    case "QUANTITY": {
      ...
      if (!(await hasEnough(userId, cost))) {
        s.step  = null;
        s.order = null;
        return ctx.replyWithHTML(
      ...
      let newBal;
      try {
        newBal = await deduct(userId, cost);
      } catch (walletErr) {
        s.order = null;
      ...
      try {
        ...
        await trackOrder(orderId, userId, cost, qty, name); // ← track for auto-refund
        s.order = null;
...
// /admin credit <userId> <amount>  — manually credit a user's wallet
// /admin balance <userId>          — check any user's balance
...
bot.command("admin", async (ctx) => {
  ...
  if (sub === "credit") {
    ...
    if (!targetId || isNaN(targetId) || !amount || isNaN(amount) || amount <= 0) {
      return ctx.replyWithHTML("⚠️ Usage: <code>/admin credit &lt;userId&gt; &lt;amount&gt;</code>");
    }

    const newBal = await credit(targetId, amount);
    await ctx.replyWithHTML(
      `✅ <b>Credited</b>
...
` +
        `<b>₦${amount.toFixed(2)}</b> has been added to your wallet by admin.
` +
    ...
    if (!targetId || isNaN(targetId)) {
      return ctx.replyWithHTML("⚠️ Usage: <code>/admin balance &lt;userId&gt;</code>");
    }
...
Balance : <b>₦${bal.toFixed(2)}</b>`
    ...
    if (!targetId || isNaN(targetId) || !amount || isNaN(amount) || amount <= 0) {
      return ctx.replyWithHTML("⚠️ Usage: <code>/admin deduct &lt;userId&gt; &lt;amount&gt;</code>");
    }

    try {
      const newBal = await deduct(targetId, amount);
      await ctx.replyWithHTML(
        `✅ <b>Deducted</b>
...
` +
      `<code>/admin credit &lt;userId&gt; &lt;amount&gt;</code>
` +
      `<code>/admin deduct &lt;userId&gt; &lt;amount&gt;</code>
` +
      `<code>/admin balance &lt;userId&gt;</code>`
    );
  }
});
...
// ═══════════════════════════════════════════════════════════════════════════════
// Launch — DB first, then Express server + Telegram bot
// ═══════════════════════════════════════════════════════════════════════════════
